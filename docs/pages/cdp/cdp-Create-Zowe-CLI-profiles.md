...
The CICS profile identifies the connection to the CICSPlex SM Web User Interface (CPSM WUI) server to query application resources. You need to know the following from your CICS system administrator:
...
| host | Host name of the CPSM WUI server. |
| port | Port number of the CPSM WUI server. |
| user | User ID to identify yourself to the CPSM WUI server . |
| password | Password to identify yourself to the CPSM WUI server. |

For example, to create a CICS profile:
